...
export class MetersComponent implements OnInit {
  ...
  this.meterService.getMeters(this.page.search, this.page.pageNumber + 1, this.page.size).pipe(takeUntil(this.unsubscribleAll)).subscribe((response:any) => {
...
onDeleteMeter(row) 
{
  ...
  Swal.fire({
    ...
    this.blockUI.start();
    this.meterService.deleteMeters(row.id).pipe(takeUntil(this.unsubscribleAll)).subscribe(response => {
      this.setPage({offset: 0 });
      Swal.fire({
        title: 'Great!',
